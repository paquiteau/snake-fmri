...
# %%
phantom = Phantom.from_brainweb(sub_id=4, sim_conf=sim_conf, tissue_file="tissue_1T5", output_res=1)
...
def resample_and_display(vx, vy, vz, sx, sy, sz, ox, oy, oz, rx, ry, rz):
    ...
    new_contrast = apply_affine(contrast, new_affine=fov_conf.affine,  old_affine=affine_orig, new_shape=fov_conf.shape, use_gpu=True).squeeze()
...
offset_sliders = VBox([
    ...
    widgets.FloatSlider(min=-200, max=0, step=1, value=-110, description='Offset Y'),
    widgets.FloatSlider(min=-200, max=0, step=1, value=-20, description='Offset Z')
])

rotation_sliders = VBox([
    widgets.FloatSlider(min=-180, max=180, step=5, value=0, description='Rotation X'),
    widgets.FloatSlider(min=-180, max=180, step=5, value=0, description='Rotation Y'),
    widgets.FloatSlider(min=-180, max=180, step=5, value=0, description='Rotation Z')
])

ui = HBox([voxel_sliders, shape_sliders, offset_sliders, rotation_sliders])

interactive_widget = widgets.interactive_output(
    resample_and_display,dict(
    vx=voxel_sliders.children[0], vy=voxel_sliders.children[1], vz=voxel_sliders.children[2],
    sx=shape_sliders.children[0], sy=shape_sliders.children[1], sz=shape_sliders.children[2],
    ox=offset_sliders.children[0], oy=offset_sliders.children[1], oz=offset_sliders.children[2],
    rx=rotation_sliders.children[0], ry=rotation_sliders.children[1], rz=rotation_sliders.children[2]
))
